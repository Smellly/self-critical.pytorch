...
class DataLoader(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        #               reverse=True))
        fc_batch, att_batch, scene_batch, label_batch, gts, infos = \
            zip(*sorted(
                    zip(
                        fc_batch, 
                        att_batch, 
        ...
        data['att_feats'] = np.zeros(
                ...
                dtype = 'float32')
        data['scene_feats'] = np.stack(
                reduce(lambda x,y:x+y, [[_]*seq_per_img for _ in scene_batch]))
