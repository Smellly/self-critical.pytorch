...
from .AttModel import pack_wrapper, AttModel, Scene3AttModel
...
class AttSceneEnsemble(Scene3AttModel):
    def __init__(self, models):
        CaptionModel.__init__(self)
        # super(AttEnsemble, self).__init__()
    ...
    def get_logprobs_state(
        ...
        # tmp = self.core(
        output_, state, _ = self.core(
                xt, 
                tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_scene_feats,
                state, tmp_att_masks)
        # for i in range(len(tmp)):
        #     print(i, len(tmp[i][0]), type(tmp[i][0]))
        output = zip(*output_)[-1]
        logprobs = torch.stack([F.softmax(m.logit(output[i]), dim=1) for i,m in enumerate(self.models)], 2).mean(2).log()
    ...
    def _prepare_feature(self, fc_feats, att_feats, scene_feats, att_masks):
        ...
        fc_feats = [m.fc_embed(fc_feats) for m in self.models]
        att_feats = [pack_wrapper(m.att_embed, att_feats[...,:m.att_feat_size], att_masks) for m in self.models]
        scene_feats = [torch.diag_embed(torch.gt(scene_feats, 0)).float()] * len(self.models)

        # Project the attention feats first to reduce memory and computation comsumptions.
        p_att_feats = [m.ctx2att(att_feats[i]) for i,m in enumerate(self.models)]

        return fc_feats, att_feats, p_att_feats, [att_masks] *len(self.models), scene_feats

    def _sample_beam(self, fc_feats, att_feats, scene_feats, att_masks=None, opt={}):
        beam_size = opt.get('beam_size', 10)
        batch_size = fc_feats.size(0)

        fc_feats, att_feats, p_att_feats, att_masksm, scene_feats \
        ...
        for k in range(batch_size):
            state = self.init_hidden(beam_size)
            tmp_fc_feats = [fc_feats[i][k:k+1].expand(
                beam_size, fc_feats[i].size(1)) \
                        for i,m in enumerate(self.models)]
            tmp_att_feats = [att_feats[i][k:k+1].expand(
                *((beam_size,)+att_feats[i].size()[1:])).contiguous() \
                        for i,m in enumerate(self.models)]
            tmp_p_att_feats = [p_att_feats[i][k:k+1].expand(
                *((beam_size,)+p_att_feats[i].size()[1:])).contiguous() \
                        for i,m in enumerate(self.models)]
            tmp_att_masks = [att_masks[k:k+1].expand(
                *((beam_size,)+att_masks.size()[1:])).contiguous() \
                        for i,m in enumerate(self.models)] if att_masks[0] is not None else att_masks
            tmp_scene_feats = [scene_feats[i][k:k+1].expand(
                *((beam_size,)+scene_feats[i].size()[1:])).contiguous() \
                        for i,m in enumerate(self.models)]
